...
def multinomial_train(X, y, C, 
    ...
    for i in range(max_iterations):
        prediction = (np.matmul(w,X.T)).T+b

        prediction = get_softmax(prediction)
        ...
        w-= gradient*step_size
        b-= bias_gradient*step_size
...
def multinomial_predict(X, w, b):
    ...
    preds= get_softmax((np.matmul(w,X.T)).T+b)
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(C):
        ...
        w[i],b[i] = binary_train(X,ytemp,w0,b0,step_size,max_iterations)

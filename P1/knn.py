...
def compute_distances(Xtrain, X):
	...
	#print(Xtrain)
	#print(X)
	#dists= np.linalg.norm(X-Xtrain)

	dists= np.zeros((X.shape[0],Xtrain.shape[0]))
	for i in range(len(X)):
		
		for j in range(len(Xtrain)):
			dists[i,j]= np.linalg.norm(X[i]-Xtrain[j])#np.sqrt(np.sum((X[i]-Xtrain[j])**2))
...
def predict_labels(k, ytrain, dists):
	...
	#				 YOUR CODE HERE					                    #
	#####################################################
	num_train= ytrain.shape[0]
	num_test= dists.shape[0]
	ypred= np.zeros(num_test,)

	import operator
	for j in range(num_test):
		neighbors_dist = [(dists[j,i],i) for i in range(num_train)]
		neighbors_dist.sort(key=lambda tup: tup[0])

		neighbors_labels = {}
		for index in range(k):
			label= ytrain[neighbors_dist[index][1]]
			#print(label)
			if label in neighbors_labels:
				neighbors_labels[label]+=1
			else:
				neighbors_labels[label]=1
		max_val= max(neighbors_labels.values())
		neighbors_max= [k for k in neighbors_labels.keys() if neighbors_labels[k]==max_val]

		ypred[j]= sorted(neighbors_max)[0]

...
def objective_function(X, y, w, lamb):
    ...
    # Converting from (n,1) to (n,) [Single dimensional]
    pred = 1 - y* np.squeeze(X.dot(w), axis=1)

    

    pred[pred < 0]=0

    obj_value = np.mean(pred)+(lamb/2 )* wnorm
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        pred= np.squeeze(X_t.dot(w),axis=1)
        pred = y_t*pred
        ...
        pred_sum = np.sum(np.expand_dims(y_ind_t, axis=1) * X_ind_t, axis=0)


        
        w+= (l_rate/k)*np.expand_dims(pred_sum,axis=1)

        wnorm = np.sqrt(np.sum(w**2))

        min_multiplier = min(1, (1/(np.sqrt(lamb)*wnorm)))
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    ypred = np.squeeze(Xtest.dot(w_l), axis=1)
    ...
    test_acc = np.mean(np.array(ypred) == np.array(ytest))
